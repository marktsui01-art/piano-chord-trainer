...
function generateArpeggioPattern(root: string, mode: KeyMode, length: number): MelodicPattern {
  const scale = getScaleForKey(root, mode);
  const startIndex = 0;
  const rootOctave = 4;
...
function generateStepwisePattern(root: string, mode: KeyMode, length: number): MelodicPattern {
  const scale = getScaleForKey(root, mode);
  const startIndex = 0;
  const rootOctave = 4;
...
function generateRandomMelody(root: string, mode: KeyMode, length: number): MelodicPattern {
  const scale = getScaleForKey(root, mode);
  const startIndex = 0;
  const rootOctave = 4;
  const startDegrees = [0, 2, 4];
  let currentScaleDegree =
    startIndex + startDegrees[Math.floor(Math.random() * startDegrees.length)];
  const notes: { name: NoteName; octave: number }[] = [];
  ...
  for (let i = 1; i < length; i++) {
    const r = Math.random();
    let interval = 0;
    if (r < 0.6) interval = Math.random() > 0.5 ? 1 : -1;
